                ...
                <table class="table table-striped">
                    ...
                    <tbody>
                        <tr>
                            ...
                            <td
                                style="color:{{ $item->role === 'admin' ? 'rgb(0, 255, 0)' : 'black' }}; font-weight: bold;">
                                {{ $item->role }}
                            </td>
                            <td>{{ $item->email }}</td>
                            <td>
                                {{-- Selalu tampilkan formulir pengubahan role --}}
                                <form onsubmit="return confirm('Yakin ingin mengubah role USER?')" class="d-inline"
                                    action="{{ route('uprole.update', $item->id) }}" method="POST">
                                    @csrf
                                    @method('PUT')
                                    <select name="role">
                                        <option value="admin" {{ $item->role == 'admin' ? 'selected' : '' }}>Admin
                                        </option>
                                        <option value="instructor" {{ $item->role == 'instructor' ? 'selected' : ''
                                            }}>Instructor</option>
                                        <option value="user" {{ $item->role == 'user' ? 'selected' : '' }}>User</option>
                                    </select>
                                    <button type="submit"
                                        class="btn-sm text-decoration-none border border-warning text-warning">Ubah
                                        Role</button>
                                </form>
